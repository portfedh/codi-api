...
describe("checkDigit", () => {
  test("should return -6 if digitoVerificadorCliente is not a number", () => {
    const resultado = {
      cadenaInformacion: { digitoVerificadorCliente: "not a number" },
    };
    expect(verifyDigit(resultado)).toBe(-6);
  });

  test("should return -6 if digitoVerificadorCliente is less than 0", () => {
    const resultado = { cadenaInformacion: { digitoVerificadorCliente: -1 } };
    expect(verifyDigit(resultado)).toBe(-6);
  });

  test("should return -6 if digitoVerificadorCliente is greater than 999999999", () => {
    const resultado = {
